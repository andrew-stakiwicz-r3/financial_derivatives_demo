...
public class YearOnYearInflationSwapHelper : YoYHelper {
  ...
  public YearOnYearInflationSwapHelper(QuoteHandle quote, Period lag, Date maturity, Calendar calendar, BusinessDayConvention bdc, DayCounter dayCounter, YoYInflationIndex index) : this(NQuantLibcPINVOKE.new_YearOnYearInflationSwapHelper__SWIG_1(QuoteHandle.getCPtr(quote), Period.getCPtr(lag), Date.getCPtr(maturity), Calendar.getCPtr(calendar), (int)bdc, DayCounter.getCPtr(dayCounter), YoYInflationIndex.getCPtr(index)), true) {
    if (NQuantLibcPINVOKE.SWIGPendingException.Pending) throw NQuantLibcPINVOKE.SWIGPendingException.Retrieve();
  }
